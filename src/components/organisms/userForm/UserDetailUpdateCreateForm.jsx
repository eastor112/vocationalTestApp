...
import { updateActiveUserAction } from '../../../context/actions/usersActions';

const UserDetailUpdateCreateForm = () => {
  const { activeUser, countries } = useSelector((state) => state.users);
  const dispatch = useDispatch();
  const { formValues, handleFormChange, setFormValues } = useForm(activeUser);
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    dispatch(updateActiveUserAction(formValues));
  };

  return (
    <form
      className='m-auto bg-white px-8 pt-6 pb-8 rounded-sm'
      ...
      <div className='grid xl:grid-cols-2 xl:gap-6 mb-7 xl:mb-2'>
        ...
        <div className='flex h-10 gap-2'>
          <figure className='h-10 w-10 overflow-hidden flex justify-center items-center'>
            <img
              src={activeUser.profile ? activeUser.profile : 'https://via.placeholder.com/150'}
              alt='profile'
            />
          </figure>
...
UserDetailUpdateCreateForm.propTypes = {

};

export default UserDetailUpdateCreateForm;
