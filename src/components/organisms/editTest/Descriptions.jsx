import React, { useState } from 'react';
import PropTypes from 'prop-types';
import DragAndDropList from '../dragAndDropList/DragAndDropList';
import { useForm } from '../../../hooks/useForm';
import { updateDescriptionsTest } from '../../../services/vocationalServices';

const Descriptions = ({ testId, descriptions }) => {
  const { formValues, handleFormChange } = useForm({
    A: descriptions.A,
    B: descriptions.B,
    C: descriptions.C,
    D: descriptions.D,
    E: descriptions.E,
  });

  const handleSubmit = (e) => {
    e.preventDefault();
    updateDescriptionsTest(testId, formValues).then(console.log);
  };
  return (
    <div className='mt-8'>

      <form onSubmit={handleSubmit}>
        <div className='mt-8'>
          <div aria-label='test description' className='relative top-1'>
            <label
              htmlFor='descriptionA'
              className='absolute -top-5 -left-2 block mb-2 text-sm text-blue-600 dark:text-gray-400 scale-75'
            >
              Description A:
            </label>
            <textarea
              id='descriptionA'
              rows='3'
              className='block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-md border border-gray-300 focus:ring-blue-500 focus:border-blue-500'
              placeholder='This career is about...'
              name='A'
              value={formValues.A}
              onChange={handleFormChange}
            />
          </div>
        </div>

        <div className='mt-8'>
          <div aria-label='test description' className='relative top-1'>
            <label
              htmlFor='descriptionB'
              className='absolute -top-5 -left-2 block mb-2 text-sm text-blue-600 dark:text-gray-400 scale-75'
            >
              Description B:
            </label>
            <textarea
              id='descriptionB'
              rows='3'
              className='block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-md border border-gray-300 focus:ring-blue-500 focus:border-blue-500'
              placeholder='This career is about...'
              name='B'
              value={formValues.B}
              onChange={handleFormChange}
            />
          </div>
        </div>

        <div className='mt-8'>
          <div aria-label='test description' className='relative top-1'>
            <label
              htmlFor='descriptionC'
              className='absolute -top-5 -left-2 block mb-2 text-sm text-blue-600 dark:text-gray-400 scale-75'
            >
              Description C:
            </label>
            <textarea
              id='descriptionC'
              rows='3'
              className='block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-md border border-gray-300 focus:ring-blue-500 focus:border-blue-500'
              placeholder='This career is about...'
              name='C'
              value={formValues.C}
              onChange={handleFormChange}
            />
          </div>
        </div>

        <div className='mt-8'>
          <div aria-label='test description' className='relative top-1'>
            <label
              htmlFor='descriptionD'
              className='absolute -top-5 -left-2 block mb-2 text-sm text-blue-600 dark:text-gray-400 scale-75'
            >
              Description D:
            </label>
            <textarea
              id='descriptionD'
              rows='3'
              className='block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-md border border-gray-300 focus:ring-blue-500 focus:border-blue-500'
              placeholder='This career is about...'
              name='D'
              value={formValues.D}
              onChange={handleFormChange}
            />
          </div>
        </div>

        <div className='mt-8'>
          <div aria-label='test description' className='relative top-1'>
            <label
              htmlFor='descriptionE'
              className='absolute -top-5 -left-2 block mb-2 text-sm text-blue-600 dark:text-gray-400 scale-75'
            >
              Description E:
            </label>
            <textarea
              id='descriptionE'
              rows='3'
              className='block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-md border border-gray-300 focus:ring-blue-500 focus:border-blue-500'
              placeholder='This career is about...'
              name='E'
              value={formValues.E}
              onChange={handleFormChange}
            />
          </div>
        </div>

        <div className='my-4'>
          <button
            type='submit'
            className='text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800'
          >
            Update
          </button>
        </div>
      </form>
    </div>
  );
};
Descriptions.propTypes = {
  testId: PropTypes.string.isRequired,
  // eslint-disable-next-line react/forbid-prop-types
  descriptions: PropTypes.object.isRequired,
};
export default Descriptions;
